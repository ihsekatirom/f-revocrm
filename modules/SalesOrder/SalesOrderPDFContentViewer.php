		...
		foreach($this->cells as $cellName => $cellWidth) {
			if($cellName == 'Name' ) {
				foreach($this->labelModel->get($cellName) as $cellLabel => $descript) {
					...
					$pdf->SetFont('','');
					$pdf->SetFontSize(8);
					$pdf->MultiCell($cellWidth, $this->headerRowHeight*2/3, $descript, 0, 'L', 0, 0, $contentFrame->x+$offsetX, $contentFrame->y+$this->headerRowHeight*1/3);
					$pdf->SetFont('','B');
					$pdf->SetFontSize(12);
				}
			}else {
	...
	function displayPreLastPage($parent) {
		...
		if ($this->contentSummaryModel) {
			...
			foreach($this->cells as $cellName=>$cellWidth) {
				...
				else $cellHeights[$cellName] = $contentFrame->h - $summaryTotalHeight+$cellOffsetY;
				++$cellIndex;
			}
		}
